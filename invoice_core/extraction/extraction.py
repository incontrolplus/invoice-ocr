...
from invoice_core.constants import RECIPIENT_KEYWORDS, SUPPLIER_KEYWORDS
...
def _is_metro_document(tokens: list[OcrToken], lines: list[LogicalLine] | None = None) -> bool:
    ...
    other_suppliers = [
        p.eik for p in get_vendor_profiles().values() if p.id != "metro" and "121644" not in (p.eik or "") and "metro" not in p.id.lower()
    ] + [
        k.upper() for p in get_vendor_profiles().values() if p.id != "metro" and "121644" not in (p.eik or "") and "metro" not in p.id.lower() for k in p.keywords
...
def extract_signatories(
    ...
    from invoice_core.accounting.legal_compliance import extract_signatories_from_text
